...
import { Box, Button, Flex, InputGroup, InputLeftElement, Icon, Stack, useMediaQuery, useBoolean } from '@chakra-ui/react';
import { ArrowUpRight, Filter, Search, X } from 'lucide-react';
import { options, optionsGrades, optionsLevels, optionsPayments, optionsStates } from '../mock/mock-filters';
...
const Filters: FC = () => {
  ...
  return (
    <Stack w="full" direction={{ base: "column", md: "row" }} justifyContent="space-between" alignItems="flex-start" spacing={3.5}>
      {!isDesktop && (
        <>
          <InputSearch />
          <Button onClick={setDisclosure.toggle} w="full" colorScheme="grayIron" leftIcon={<Filter size={20}/>}>Filtrar</Button>
        </>
      )}

      <Flex hidden={isDesktop ? false : disclosure} w="full" direction={{ base: "column", md: "row" }} flexWrap="wrap" gap={3}>
        ...
        <Box w={{ base: "full", md: "auto" }}>
          <Button w={{ base: "full", md: "auto" }} colorScheme="blackAlpha" variant="link" leftIcon={<X />} lineHeight="44px">
            Borrar filtros
          </Button>
        </Box>
      </Flex>

      <Button w={{ base: "full", md: "auto" }} colorScheme="grayIron" leftIcon={<ArrowUpRight size={20} />}>
        Exportar
      </Button>
    </Stack>
  )
