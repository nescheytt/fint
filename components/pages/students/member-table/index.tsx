          ...
          {mockData.map((member) => {
            return (
              <Tr key={member.id} _hover={{ bg: 'grayIron.50' }}>
                ...
                <Td w="12%" py={0} px={5} cursor="pointer" onClick={() => handleOnClick(member)}>
                  <Text color="gray.600" textTransform="capitalize">{member.student.level}</Text>
                </Td>
                <Td w="11%" py={0} px={5} cursor="pointer" onClick={() => handleOnClick(member)}>
                  <Text color="gray.600" textTransform="capitalize">{member.student.shift}</Text>
                </Td>
                <Td w="11%" py={0} px={5} cursor="pointer" onClick={() => handleOnClick(member)}>
                  <Text color="gray.600" textTransform="capitalize">{member.student.grade}</Text>
                </Td>
                <Td w="12%" p="10px 10px 10px 20px" cursor="pointer" onClick={() => handleOnClick(member)}>
                  <Badge size="sm" colorScheme={setColorSchemeBadge(member.student.status.id)} textTransform="capitalize">{member.student.status.label}</Badge>
                </Td>
                <Td w="4%" p={0}>
