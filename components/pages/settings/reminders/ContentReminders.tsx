...
const ContentReminders: React.FC<Props> = ({ data }) => {
  return (
    <Stack divider={<Divider />} spacing={3.5}>
      {data.map((item, index) => {
        return (
          <Box key={index} p={{ base: 3, md: 6 }} bgColor='white' boxShadow='xs' border="1px solid" borderColor='gray.200' borderRadius='xl'>
            <HStack justifyContent='space-between'>
              <HStack spacing={3}>
                ...
                <Text>Enviar <b>{renderTextType(item.type)} de recordatorio, {item.date} día antes</b> de la fecha <b>de vencimiento</b></Text>
              </HStack>
