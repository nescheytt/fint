...
import { FormControl, FormLabel, Grid, GridItem, HStack, Stack, SimpleGrid, Text } from "@chakra-ui/react";
    ...
    <Formik initialValues={{}} onSubmit={(values, actions) => handlerSubmit({ values, actions })}>
      <Form>
        <Stack spacing={6}>
              ...
              <Field name='type'>
                {({ field }: any) => (
                  <FormControl>
                    ...
                    <SelectTheme {...field} ref={initialRef} options={optionsTypesReminders} placeholder="WhatsApp" />
                  </FormControl>
                )}
              </Field>
            </GridItem>
              ...
              <Field name='quantity'>
                {({ field }: any) => (
                  <FormControl>
                    <FormLabel>Tipo de recordatorio</FormLabel>
                    <HStack spacing={0}>
                      <InputTheme {...field} type="number" placeholder="0" maxWidth="75px" borderRightRadius={0} />
                      <SelectTheme {...field} ref={initialRef} options={optionsDateReminders} placeholder="Dias después de la fecha de vencimiento" borderLeftRadius={0} />
                    </HStack>
                  </FormControl>
                )}
              </Field>
          ...
          <Stack spacing={5}>
            <Text fontSize="md" fontWeight={600}>Mensaje</Text>
              ...
              <Field name='subject'>
                {({ field }: any) => (
                  <FormControl>
                    ...
                    <InputTheme {...field} type="text" placeholder="Mejora la gestión de cobros y pagos de [company]" />
                  </FormControl>
                )}
              </Field>
